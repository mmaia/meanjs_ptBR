...
#uncompress, delete compressed and move nodejs to ~/tools changing it's name to node-v0.12.9
tar -xvf node-v0.12.9-linux-x64.tar.gz
rm -rf node-v0.12.9-linux-x64.tar.gz
mkdir ~/tools
mv node-v0.12.9-linux-x64 ~/tools/node-v0.12.9

#add node and npm to .bashrc
echo 'export PATH=$PATH:~/tools/node-v0.12.9/bin:~/npm-global/bin' >> ~/.bashrc
...
mkdir ~/npm-global
~/tools/node-v0.12.9/bin/npm config set prefix '~/npm-global'
